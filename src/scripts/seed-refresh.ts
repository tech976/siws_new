...
const STEPS: Step[] = [
  ...
  /*
   * Straight after the library, and before anything that publishes a page.
   * The permission records are database-only — this file's own header says
   * why — so without this step every photograph of a child is held back and
   * the pages seed around the gap rather than failing loudly.
   */
  { script: 'record:consent', does: 'the parental permission records' },
  {
    script: 'seed:image-filing',
    does: 'photographs filed under the section they actually show',
  },
   ...
   * `nav.ts` can place it as a CHILD entry inside About. Run the other way
   * round, the gallery page climbs out of the drop-down into the top row —
   * which is what put the Secondary menu on two lines.
   *
   * `seed:nav` is last in this list for the same reason it is last in the
   * README: it reads whatever every step above it produced.
   */
