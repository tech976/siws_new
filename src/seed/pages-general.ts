...
interface Recipe {
   ...
   * the Student Wall, whose whole purpose is to show pupils' work, sat as a
...
const RECIPES: Record<string, Recipe> = {
  ...
  "student-wall": {
    ...
    /*
     * No outro. It read "Work is published with the pupil's name only where the
     ...
     * the page look like a policy notice rather than a wall of pupils' work.
     * The rule has not gone anywhere; it simply stopped being the page.
     */
    gallery: {
      heading: "Work from our pupils",
      accentWord: "our pupils",
      categories: [
        "Student Activities",
        "Handson Experience",
...
const run = async () => {
  ...
  for (const page of blank) {
     ...
     */
    const banner = wanted
      ? (publishable.find((m) => baseName(String(m.filename)) === wanted)?.id ??
        null)
      : null;
              ...
              ? [
                  {
                    ...
                    ...(recipe.gallery.accentWord
                      ? { accentWord: recipe.gallery.accentWord }
                      : {}),
                    headingLevel: "h2",
                    layout: "grid",
                    perPage: "12",
